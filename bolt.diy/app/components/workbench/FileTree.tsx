...
function ContextMenuItem({ onSelect, children }: { onSelect?: () => void; children: ReactNode }) {
  return (
    <ContextMenu.Item
      onSelect={onSelect}
      className="flex items-center gap-2 px-2 py-1.5 outline-0 text-sm text-codez-elements-textPrimary cursor-pointer ws-nowrap text-codez-elements-item-contentDefault hover:text-codez-elements-item-contentActive hover:bg-codez-elements-item-backgroundActive rounded-md"
    >
...
function InlineInput({ depth, placeholder, initialValue = '', onSubmit, onCancel }: InlineInputProps) {
  ...
  return (
    <div
      className="flex items-center w-full px-2 bg-codez-elements-background-depth-4 border border-codez-elements-item-contentAccent py-0.5 text-codez-elements-textPrimary"
      style={{ paddingLeft: `${6 + depth * NODE_PADDING_LEFT}px` }}
    >
      <div className="scale-120 shrink-0 i-ph:file-plus text-codez-elements-textTertiary" />
      <input
        ref={inputRef}
        type="text"
        className="ml-2 flex-1 bg-transparent border-none outline-none py-0.5 text-sm text-codez-elements-textPrimary placeholder:text-codez-elements-textTertiary min-w-0"
    ...
    <>
      <ContextMenu.Root>
        <ContextMenu.Trigger>
          <div
            ...
            className={classNames('relative', {
              'bg-codez-elements-background-depth-2 border border-dashed border-codez-elements-item-contentAccent rounded-md':
                isDragging,
            })}
          >
            {children}
        ...
        <ContextMenu.Portal>
          <ContextMenu.Content
            style={{ zIndex: 998 }}
            className="border border-codez-elements-borderColor rounded-md z-context-menu bg-codez-elements-background-depth-1 dark:bg-codez-elements-background-depth-2 data-[state=open]:animate-in animate-duration-100 data-[state=open]:fade-in-0 data-[state=open]:zoom-in-98 w-56"
          >
            <ContextMenu.Group className="p-1 border-b-px border-solid border-codez-elements-borderColor">
            ...
            <ContextMenu.Group className="p-1 border-t-px border-solid border-codez-elements-borderColor">
              {!isFolder ? (
                <>
    ...
    <FileContextMenu onCopyPath={onCopyPath} onCopyRelativePath={onCopyRelativePath} fullPath={folder.fullPath}>
      <NodeButton
        className={classNames('group', {
          'bg-transparent text-codez-elements-item-contentDefault hover:text-codez-elements-item-contentActive hover:bg-codez-elements-item-backgroundActive':
            !selected,
          'bg-codez-elements-item-backgroundAccent text-codez-elements-item-contentAccent': selected,
        })}
        depth={folder.depth}
...
function File({
  ...
  return (
    <FileContextMenu onCopyPath={onCopyPath} onCopyRelativePath={onCopyRelativePath} fullPath={fullPath}>
      <NodeButton
        className={classNames('group', {
          'bg-transparent hover:bg-codez-elements-item-backgroundActive text-codez-elements-item-contentDefault':
            !selected,
          'bg-codez-elements-item-backgroundAccent text-codez-elements-item-contentAccent': selected,
        })}
        depth={depth}
        iconClasses={classNames('i-ph:file-duotone scale-98', {
          'group-hover:text-codez-elements-item-contentActive': !selected,
        })}
        onClick={onClick}
      >
        <div
