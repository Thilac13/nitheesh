...
export function RepositoryList({ repos, isLoading, onSelect, activeTab }: RepositoryListProps) {
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center py-8 text-codez-elements-textSecondary dark:text-codez-elements-textSecondary-dark">
        <StatusIndicator status="loading" pulse={true} size="lg" label="Loading repositories..." className="mb-2" />
        <p className="text-xs text-codez-elements-textTertiary dark:text-codez-elements-textTertiary-dark">
