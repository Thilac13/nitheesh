...
export class ImportExportService {
    ...
    try {
      ...
      return {
        ...
        core: {
          ...
          codez_user_profile: this._safeGetItem('codez_user_profile'),
          codez_settings: this._safeGetItem('codez_settings'),
          codez_profile: this._safeGetItem('codez_profile'),
        ...
        features: {
          // Feature flags
          viewed_features: this._safeGetItem('codez_viewed_features'),
          developer_mode: this._safeGetItem('codez_developer_mode'),
        ...
        ui: {
          // Tab configuration
          codez_tab_configuration: this._safeGetItem('codez_tab_configuration'),
        ...
        debug: {
          ...
          acknowledged_debug_issues: this._safeGetItem('codez_acknowledged_debug_issues'),
          acknowledged_connection_issue: this._safeGetItem('codez_acknowledged_connection_issue'),

          // Error logs
          error_logs: this._safeGetItem('error_logs'),
          codez_read_logs: this._safeGetItem('codez_read_logs'),
        ...
        updates: {
          ...
          last_acknowledged_update: this._safeGetItem('codez_last_acknowledged_version'),
        },
  ...
  static async deleteAllChats(db: IDBDatabase): Promise<void> {
    ...
    localStorage.removeItem('codez_chat_history');
  ...
  private static async _importComprehensiveFormat(data: any): Promise<void> {
    ...
    if (data.ui) {
      ...
      if (data.ui.codez_tab_configuration) {
        try {
          this._safeSetItem('codez_tab_configuration', data.ui.codez_tab_configuration);
        } catch (err) {
    ...
    if (data.debug) {
      ...
      const debugLocalStorageKeys = [
        'codez_acknowledged_debug_issues',
        'codez_acknowledged_connection_issue',
        'error_logs',
        'codez_read_logs',
      ];
    ...
    if (data.updates) {
      ...
      if (data.updates.last_acknowledged_update) {
        try {
          this._safeSetItem('codez_last_acknowledged_version', data.updates.last_acknowledged_update);
        } catch (err) {
