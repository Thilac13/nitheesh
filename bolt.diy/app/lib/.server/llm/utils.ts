...
export function simplifycodezActions(input: string): string {
  // Using regex to match codezAction tags that have type="file"
  const regex = /(<codezAction[^>]*type="file"[^>]*>)([\s\S]*?)(<\/codezAction>)/g;
...
export function createFilesContext(files: FileMap, useRelativePath?: boolean) {
  ...
  const fileContexts = filePaths
    ...
    .map((path) => {
      ...
      return `<codezAction type="file" filePath="${filePath}">${codeWithLinesNumbers}</codezAction>`;
    });

  return `<codezArtifact id="code-content" title="Code Content" >\n${fileContexts.join('\n')}\n</codezArtifact>`;
}
