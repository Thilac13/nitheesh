...
import { createFilesContext, extractCurrentContext, extractPropertiesFromMessage, simplifycodezActions } from './utils';
...
export async function selectContext(props: {
  ...
  const processedMessages = messages.map((message) => {
    if (message.role === 'user') {
      ...
      content = simplifycodezActions(content);

      content = content.replace(/<div class=\\"__codezThought__\\">.*?<\/div>/s, '');
...
---
  ...
  if (totalFiles == 0) {
    throw new Error(`codez failed to select files`);
  }
