  ...
  return (tree: any) => {
    visit(tree, (node: any) => {
      if (node.type === 'html' && node.value && node.value.startsWith('<think>')) {
        ...
        node.value = `<div class="__codezThought__">${cleanedContent}`;
...
const rehypeSanitizeOptions: RehypeSanitizeOptions = {
  ...
  attributes: {
    ...
    div: [
      ...
      'data*',
      ['className', '__codezArtifact__', '__codezThought__', '__codezQuickAction', '__codezSelectedElement__'],

      // ['className', '__codezThought__']
    ],
    button: [
      ...
      'name',
      'value',
      ['className', '__codezArtifact__', '__codezThought__', '__codezQuickAction'],
    ],
  },
  strip: [],
};
