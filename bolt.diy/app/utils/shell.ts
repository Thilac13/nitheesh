...
export class codezShell {
  ...
  async init(webcontainer: WebContainer, terminal: ITerminal) {
    ...
    const { process, commandStream, expoUrlStream } = await this.newcodezShellProcess(webcontainer, terminal);
  ...
  async newcodezShellProcess(webcontainer: WebContainer, terminal: ITerminal) {
    const args: string[] = [];
    const process = await webcontainer.spawn('/bin/jsh', ['--osc', ...args], {
      terminal: {
        cols: terminal.cols ?? 80,
...
export function newcodezShellProcess() {
  return new codezShell();
}
